...
let cards = [
    {
      title: `Raid Road`,
      description: "Приключенческий инди-слешер о судьбе молодого парня по имени Рейд",
      price: 10
    },
    {
      title: "По ту сторону границы",
      description: "Квест молодого агента КГБ в далёком богом-забытом Златовске",
      price: 40
    },
    {
      title: "Культурная столица - Ленинград",
      description: "Экшн-адвентчура в открытом мире изменившегося до неузнаваемости города Ленинград",
      price: 70
    },
    {
      title: "???",
  ...
  function Cart(){
    ...
    this.render = function(){
      ...
      let cnt = this.count()
      if(cnt > 0){
        shown.unshift(`<h2>Корзина: ${this.items.length} позиций на ${this.count()} $.</h2>`)
      }else{
  ...
  function showCurrentThree(){
    ...
    let shown = currentPart.map(info => {
      ...
      <div class="card">
        ...
        <button onclick="cart.add('${info.title}')">Купить / ${info.price} $</button>
      </div>
      `
    })
